...
function http_badRequest(res, data, type) {
...
terms = {
  ...
  , 'dtcontains': {
    value: function (r, field, term) {
      return toField(r, field).toISO8601().match("" + term);
    }
  }
  , 'startswith': {
    value: function (r, field, term) {
...
module.exports = RethinkResource = new Class({
  ...
  , build_error: function build_error(err, bundle) {
    if (err.name === 'ValidationError') {
      bundle.data = {code: 400, message: err.message};
      return this.respond(bundle, http_badRequest);
    } else if (err.name === 'DuplicatePrimaryKeyError') {
      bundle.data = {code: 400, message: 'Duplicate primary key'};
      return this.respond(bundle, http_badRequest);
    } else if (err.name === 'DocumentNotFoundError') {
      bundle.data = {code: 404, message: 'Not found'};
      return this.respond(bundle, http.notFound);
    } else if (err.name === 'CustomError') {
      bundle.data = {code: err.status || 500, message: err.message};
      return this.respond(bundle, (res, data, type) => res(data || STATUS_CODES[err.status || 500]).code(err.status || 500).type(type) );
    } else {
  ...
  /**
   * Top level method used to retreive individual objects by id.
  ...
  , update_object: function (bundle, callback) {
    this.get_object(bundle, function (err, obj) {
      ...
      if (!obj) {
        ...
        customErr.name = 'DocumentNotFoundError';
        return callback && callback(customErr, null);
      }
