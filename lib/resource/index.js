...
terms = {
  ...
  , 'regex': {
    value: function (r, field, term) {
      return toField(r, term).match(term);
    }
  }
  , 'iexact': {
    value: function (r, field, term) {
...
module.exports = RethinkResource = new Class({
  ...
  , build_error: function build_error(err, bundle) {
    if (err.name === 'ValidationError') {
      ...
      return that.respond(bundle, http_forbidden);
      ...
      return that.respond(bundle, http_serverError)
    }
  }
   ...
   * @param {module:tastypie/lib/resource~Bundle} bundle An object representing the current `PUT` request
   **/
